...
CloudInfo SampleCloudDensity_No3DTex(SamplingInfo dsi)
{
    ...
    //计算云密度
    float cloudDensity = GetCloudTypeDensity(heightFraction,
        min(lowerLayerHeight, upperLayerHeight), max(lowerLayerHeight, upperLayerHeight), dsi.feather);
    if (cloudDensity <= 0)
    {
        o.density = 0;
        o.absorptivity = 1;
...
CloudInfo SampleCloudDensity_RealTime(SamplingInfo dsi, bool isCheaply = true)
{
    ...
    //计算云类型密度
    float stratusDensity = GetCloudTypeDensity(heightFraction, dsi.stratusInfo.x, dsi.stratusInfo.y,
        dsi.stratusInfo.z);
    ...
    float baseShape = Remap(baseTex.r, saturate((1.0 - baseTexFBM) * dsi.baseShapeDetailEffect),
        1.0, 0, 1.0);
