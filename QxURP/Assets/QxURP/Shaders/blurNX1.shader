Shader "Hidden/blurNX1"
{
	Properties
	{
		_MainTex ("Texture", 2D) = "white" {}
	}
	SubShader
	{
		// No culling or depth
		Cull Off ZWrite Off ZTest Always

		Pass
		{
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag
			
			#include "UnityCG.cginc"
			#include "globalUniforms.cginc"
			#include "shadow.cginc"
			#include "random.cginc"

			struct appdata
			{
				float4 vertex : POSITION;
				float2 uv : TEXCOORD0;
			};

			struct v2f
			{
				float2 uv : TEXCOORD0;
				float4 vertex : SV_POSITION;
			};

			v2f vert (appdata v)
			{
				v2f o;
				o.vertex = UnityObjectToClipPos(v.vertex);
				o.uv = v.uv;
				return o;
			}
			
			sampler2D _MainTex;

			float frag (v2f i) : SV_Target
			{
				float2 uv = i.uv;

				float3 normal = tex2D(_GT1, uv).rgb * 2 - 1;
				float depth = UNITY_SAMPLE_DEPTH(tex2D(_gdepth, uv));
				float3 worldPos = mul(_vpMatrixInv, float4(uv * 2 - 1, depth, 1));
				worldPos /= worldPos.w;

				float result = 0;
				float weight = 0;
				float r = 3;
				
				float dist  = distance(_WorldSpaceCameraPos.xyz, worldPos.xyz);
				float radius = rcp(pow(dist, 1.2) * 0.01 + 0.01);

				// 这里只是horizental blur
				for (int i = -r; i <= r; ++i)
				{
					float2 offset = float2(i, 0) / float2(_screenWidth/4, _screenHeight/4);
					float2 uvSample = uv + offset * radius;
					result += tex2D(_MainTex, uvSample).r;
					weight += 1;
				}
				result /= weight;

				return result;
			}
			ENDCG
		}
	}
}